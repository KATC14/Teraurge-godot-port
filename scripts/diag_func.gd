...
func script_library(logic: String) -> Array:
	var diag_file
	var s_index

	# VARIABLES
	var next_index:Array = [null, null, null]
	var index
	...
	match slogic[0]:
		"pic":
			...
			next_index = ['create_picture', slogic[1], null]
		"remove_pic":
			VarTests.has_story = false
			#create_picture.emit()
			next_index = ['remove_pic', null, null]
		"change_sprite":
			#change_sprite.emit()
			next_index = ['change_sprite', slogic[1], null]
...
#			advance_time.emit(0)# TODO add advance_time function
...
#				map_target = previous_map_target as MovieClip
		...
		"start_combat":
			...
			#start_combat.emit()
			next_index = ['start_combat', null, null]
			#return "skip_dialogue_breaker"
# DEPRECATED
#		"combat_damage":
...
#				player_hitpoints_damage = 0
		...
		"sex_gate", "gender_gate", "sex_branch":
			...
			if VarTests.player_gender == "male":   index = slogic[1]
			if VarTests.player_gender == "female": index = slogic[2]
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"rape_filter":
			# rape_filter (pointer1) (pointer2)
			if VarTests.RAPE_FILTER: index = slogic[1]
			else:                    index = slogic[2]
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"feral_filter":
			# feral_filter (pointer1) (pointer2)
			if VarTests.FERAL_FILTER: index = slogic[1]
			else:                     index = slogic[2]
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"gore_filter":
			# gore_filter (pointer1) (pointer2)
			if VarTests.GORE_FILTER: index = slogic[1]
			else:                    index = slogic[2]
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"day_night_gate":
			...
			# night
				...
				index = slogic[2]
			print('dng next_index ', index)
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"morn_day_eve_night_gate":
			# day_night_gate (morning pointer) (day pointer) (evening pointer) (night pointer)
			# morning
			if VarTests.TIME >=  15 and VarTests.TIME < 30: index = slogic[1]
			# day
			if VarTests.TIME >=  30 and VarTests.TIME < 70: index = slogic[2]
			# evening
			if VarTests.TIME >=  70 and VarTests.TIME < 85: index = slogic[3]
			# night
			if VarTests.TIME >=  85 || VarTests.TIME < 15:  index = slogic[4]
			#change_index.emit(next_index)
			next_index = ['change_index', index, null]
		"advance_time":
			# advance_time (time amount)
			#advance_time.emit(int(slogic[1]))
			next_index = ['advance_time', int(slogic[1]), null]
		"advance_time_to":
			...
			if int(slogic[1]) != 0:
				if VarTests.TIME >= int(slogic[1]):
					slogic[1] = str((100 - VarTests.TIME) + int(slogic[1]))
				else:
					slogic[1] = str(int(slogic[1]) - VarTests.TIME)
		...
		"chance":
			...
			if randnum < int(slogic[1]): index = slogic[3]
			else:                        index = slogic[2]
			next_index = ['chance', index, null]
		"random_pointer":
			...
			next_index = ['change_index', r_pointer, null]
		"discover_location":
		...
		"check_counter":
			...
			if VarTests.COUNTERS.find(slogic[1]) != -1:
				...
				if 0 >= slogic[2]:
					...
					index = slogic[3]
			next_index = ['check_counter', index, null]
			#TEST THIS!!
		"remove_counter":
		...
		"check_discovery":
			...
			if VarTests.DISCOVERED_LOCATIONS.find(slogic[1]) != -1:
				...
				index = slogic[2]
			next_index = ['check_discovery', index, null]
			#should work ^
...
#			hub_location = true
# TODO add in curated_list from diag
#		"curated_list":
...
#				curated_list_next = slogic[1]
		"curated_list":
			...
			next_index = ['curated_list', slogic[1], null]
		"auto_continue":
